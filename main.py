...
def work():
    ...
    try:
        ...
        if platform.system().lower() == 'windows':
            ...
            tab_num = cpu_count * 2 - 1
        for index_1 in range(tab_num):
                ...
                for index_2, page in enumerate(tab_list):
                    ...
                    if off_button_ele:
                        ...
                        if cycles_text_ele and nex_points_ele:
                            ...
                            logging.info(f"[邮箱 {mail} 地址 {address} 标签页{index_2+1} 当前Cycles/s 为 {cycles_text}, 总 Nex points 为 {nex_points_text}]")
                    time.sleep(5)
                    if 'unknow' == mail:
                        auth_token = page.local_storage('dynamic_authentication_token')
                        if auth_token:
